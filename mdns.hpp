...
struct service_t {
	struct {
		mdns_record_t* txt_record = nullptr;
		size_t txt_record_count = 0;
		mdns_record_t& operator[](size_t index) const {
			if (index >= txt_record_count) {
				throw std::out_of_range("Index out of range");
			}
			return txt_record[index];
		}
	} txt_record;
...
class MDNSService {
   public:
	MDNSService(const std::string& hostname, const std::string& serviceName, int servicePort,
	            std::map<const char*, const char*> txtRecords);

	~MDNSService();

	// returns the return value of the c call
   ...
   private:
	...
	int servicePort_;
	std::map<const char*, const char*> txtRecords_;
...
int
MDNSCPP_queryCallback(int sock, const struct sockaddr* from, size_t addrlen,
                      mdns_entry_type_t entry, uint16_t query_id, uint16_t rtype, uint16_t rclass,
                      uint32_t ttl, const void* data, size_t size, size_t name_offset,
...
struct ServiceFound {
	std::string instance_service_name;
	std::string host_name;
	std::optional<std::string> ipv4_addr;
	std::optional<std::string> ipv6_addr;
	std::optional<uint16_t> port;
	std::optional<uint16_t> priority;
	std::optional<uint16_t> weight;
	std::optional<std::unordered_map<std::string, std::string>> txtRecords;
};

// TODO: Actually use ttl to refresh the data
class MDNSClient {
   public:
	MDNSClient(const std::string& service_name);
	~MDNSClient();

	// void listenForServices();
	// void stopListeningForServices();

	// std::vector<ServiceFound> getServicesFound();

	// void onServiceFound(MoveOnlyFunction<void(ServiceFound)>&& onServiceFound);

	// TODO: Instead of asking to wether wait or no for a record, check how many additional records
	// there are and wait for them. For that you should see `mdns_query_recv`
	std::future<std::optional<ServiceFound>> findService(int timeout_seconds, bool wait_for_txt,
	                                                     bool wait_for_bothIP46);
   ...
   private:
	// std::thread listenForServicesThread_;
	std::thread goodbyeThread_;
	std::string service_name_;
	std::string goodbye_service_instance_string_;

	// std::map<std::pair<std::string, std::string>, ServiceFound> servicesFound_;

	// MoveOnlyFunction<void(ServiceFound)> onServiceFound_;
};
